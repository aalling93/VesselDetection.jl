...
function filter_sobel_test()
    ...
    sobel = VesselDetection.Ship_detector.sobel_filter(image_with_objects);
...
function filter_mean_test()
    ...
    mean = VesselDetection.Ship_detector.mean_filter([7,7]);
    check &= isapprox(sum(mean),1; atol = 0.1);
    check &= size(mean)==(7,7);
    #checking the mean filter applied on the image.
    mean_no_padding = VesselDetection.Ship_detector.conv2d(image_with_objects, VesselDetection.Ship_detector.mean_filter([3,3]));
    mean_with_padding = VesselDetection.Ship_detector.conv2d(image_with_objects, VesselDetection.Ship_detector.mean_filter([3,3]),1,"same");
